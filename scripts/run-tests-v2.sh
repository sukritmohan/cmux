...
APP="$DERIVED_DATA_PATH/Build/Products/Debug/Beethoven DEV.app"
RUN_TAG="tests-v2"
...
if [ ! -d "$APP" ]; then
  echo "ERROR: Beethoven DEV.app not found at expected path: $APP" >&2
  exit 1
fi

cleanup() {
  pkill -x "Beethoven DEV" || true
  pkill -x "Beethoven" || true
...
launch_and_wait() {
  ...
  for _ in {1..50}; do
    pgrep -x "Beethoven DEV" >/dev/null 2>&1 || break
    sleep 0.1
  done
  ...
  CMUX_TAG="$RUN_TAG" CMUX_UI_TEST_MODE=1 "$APP/Contents/MacOS/Beethoven DEV" >/dev/null 2>&1 &
