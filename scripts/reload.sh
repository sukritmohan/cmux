...
APP_NAME="Beethoven DEV"
BUNDLE_ID="com.cmuxterm.app.debug"
BASE_APP_NAME="Beethoven DEV"
DERIVED_DATA=""
NAME_SET=0
BUNDLE_SET=0
DERIVED_SET=0
...
select_cmux_shim_target() {
  local app_cli_dir="/Applications/Beethoven.app/Contents/Resources/bin"
...
print_tag_cleanup_reminder() {
  ...
  if [[ "${#stale_tags[@]}" -eq 0 ]]; then
    ...
    for tag in "${stale_tags[@]}"; do
      echo "  pkill -f \"Beethoven DEV ${tag}.app/Contents/MacOS/Beethoven DEV\""
  ...
  echo "  pkill -f \"Beethoven DEV ${current_slug}.app/Contents/MacOS/Beethoven DEV\""
...
if [[ -x "$CLI_PATH" ]]; then
  ...
  write_dev_cli_shim "$DEV_CLI_SHIM" "/Applications/Beethoven.app/Contents/Resources/bin/cmux"

  CMUX_SHIM_TARGET="$(select_cmux_shim_target || true)"
  if [[ -n "${CMUX_SHIM_TARGET:-}" ]]; then
    write_dev_cli_shim "$CMUX_SHIM_TARGET" "/Applications/Beethoven.app/Contents/Resources/bin/cmux"
  fi
fi
